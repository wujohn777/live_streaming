  ...
  methods: {
    ...
    verify() {
      ...
      for (let i = 0; i < this.view.length; i++) {
        ...
        // 若為成人崇拜則檢核日期是否為週六或週日
        if ((this.person.kind == "成人崇拜") && (this.view[i].format("dddd") != "Saturday" && this.view[i].format("dddd") != "Sunday"))  {
           alert("【收看日期】" + this.view[i].format("YYYY-MM-DD") +  "必須是週六或週日，請再確認！！");
           return false;
        }

	// 若為晚上合心會則檢核日期是否為週三
        if ((this.person.kind == "晚上合心會") && (this.view[i].format("dddd") != "Wednesday"))  {
           alert("【收看日期】" + this.view[i].format("YYYY-MM-DD") +  "必須是週三，請再確認！！");
           return false;
        }
    ...
    async submit() {
      ...
      if (this.verify()) {
        ...
        if (record) {
           ...
           await fetch(this.gas2 + '?uuid=' + uuid + '&kind=' + this.person.kind + '&date=' + view_date.value, {
                    method: 'POST'
                 })
                 .catch(err => {
                 });
